...
public class ServerFirstReaderInterceptor implements ReaderInterceptor {

	private static final Logger logger = getLogger(ServerFirstReaderInterceptor.class.getName());
	
	@Override
	public Object aroundReadFrom(ReaderInterceptorContext interceptorContext)
			throws IOException, WebApplicationException {
		logger.info("ServerFirstReaderInterceptor1 invoked");
		InputStream inputStream = interceptorContext.getInputStream();
		byte[] bytes = new byte[inputStream.available()];
		inputStream.read(bytes);
		String requestContent = new String(bytes);
		requestContent = requestContent + ".Request changed in ServerFirstReaderInterceptor1.";
